...
- choose the v0.1 runtime strategy through ADR-001;
...
- first skill and execution boundary are defined;
- threat model and approval path are reflected in design.

## Phase 1 — Loren v0.1: useful personal project assistant

Primary objective:

> Loren can be used every day for project-oriented work and remembers enough context to become more useful over time.

Capabilities:

- owner-authenticated chat/development interface;
- persistent Loren identity;
- initial world-model entities for projects and repositories;
- working + durable memory;
- memory inspection/correction;
- one selected model/runtime path;
- GitHub as the first external skill;
- web research capability;
- permission gateway for tool calls;
- audit log;
- internal tasks/reminders or minimal scheduler;
- model provider abstraction only to the degree required by the selected runtime strategy.

Target tasks:

```text
"Loren, check the wedding project."
"What did we decide about storage for this project?"
"Remember that production deploys require my approval."
"Inspect this repo and tell me what needs attention."
"Research the options and attach the conclusion to this project."
"Tomorrow remind me to revisit auth."
```
...
- Loren survives restart without losing identity/project context;
- memory corrections work;
- GitHub status comes from GitHub rather than model assumptions;
- all external writes pass permission policy;
- every consequential action is auditable;
- runtime/model replacement boundary is demonstrated at least through tests or a small spike;
- owner can use Loren for real project work repeatedly without manually rebuilding context each session.
