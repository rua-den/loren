...
internal static class MaintenanceCli
{
    ...
    private static async Task<int> ExportAsync(string[] args)
    {
        ...
        {
            throw new IOException("Output archive already exists.");
        }

        await using CanonicalStateDbContext source = Context(sourcePath, readOnly: true);
        if ((await source.Database.GetPendingMigrationsAsync()).Any())
        {
            throw new InvalidOperationException("Source database has unapplied migrations; migrate it before exporting.");
        }
    ...
    private static async Task<int> RestoreAsync(string[] args)
    {
        ...
        {
            throw new FileNotFoundException("Archive does not exist.", archivePath);
        }

        if (Directory.Exists(targetDirectory))
        {
            throw new IOException("Target directory must be new and absent.");
        }
    ...
    private static CanonicalStateDbContext Context(string path, bool readOnly = false)
    {
        ...
        return new CanonicalStateDbContext(
